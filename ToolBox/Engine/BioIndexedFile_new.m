classdef  BioIndexedFile_new
	% BioIndexedFile_new class allows random read access to text files using an index file.
    %
	%  A  BioIndexedFile_new object uses an auxiliary index file to store
    ...
    %
    %  A  BioIndexedFile_new object lets you access individual entries or a
    ...
    %  are read-only.
    %        
    %   BioIndexedFile_new methods:
    %     BioIndexedFile_new    - Create a  BioIndexedFile_new object.
    ...
    %     gene2go =  BioIndexedFile_new('mrtab',sourcefile,'.','KeyColumn',3,...
    %                              'HeaderPrefix','!')
    ...
    properties (SetAccess = private, GetAccess = public)
        ...
        %
        %  See also  BioIndexedFile_new,  BioIndexedFile_new. BioIndexedFile_new.
        FileFormat
        %INPUTFILE path and filename of the source file.
        ...
        %
        %  See also  BioIndexedFile_new,  BioIndexedFile_new. BioIndexedFile_new,
        %   BioIndexedFile_new.getEntryByKey.         
        IndexedByKeys
    end
    ...
    methods
        ...
        function obj =  BioIndexedFile_new_new(fileFormat,inputFile,varargin)
        % BioIndexedFile_new Create a bioinformatics indexed file object.
        %
        %   BioIndexedFile_new(FORMAT,SOURCEFILE) constructs a  BioIndexedFile_new object
        ...
        %
        %  The  BioIndexedFile_new object uses an auxiliary index file to store
        ...
        %  file does not exist, the  BioIndexedFile_new constructor creates one by
        %  analyzing SOURCEFILE, otherwise, the object is built from the information
        %  stored in the auxiliary index file.
        %
        %   BioIndexedFile_new(FORMAT,SOURCEFILE,INDEXDIR) sets a different directory to
        %  write or search for the auxiliary index file.
        %
        %   BioIndexedFile_new(FORMAT,SOURCEFILE,INDEXFILE) sets the name of the 
        %  auxiliary index file. INDEXFILE may be prefixed by a relative or
        %  absolute path.
        %
        %   BioIndexedFile_new(...,'INDEXEDBYKEYS',TF) indicates whether the object can 
        %  be accessed by keys or not. Defaults to true.
        %
        %   BioIndexedFile_new(...,'MEMORYMAPPEDINDEX',TF) indicates whether the object 
        ...
        %
        %   BioIndexedFile_new(...,'INTERPRETER',FH) sets a function handle that is to be
        ...
        %
        %   BioIndexedFile_new(...,'VERBOSE',TF) displays information about the object
        ...
        %
        %   BioIndexedFile_new(...,'KEYCOLUMN',KC) when format is 'TABLE' or 'MRTAB', KC
        %  contains an integer indicating the column of the table to be used as the
        %  keys. Defaults to the first column.
        %
        %   BioIndexedFile_new(...,'KEYTOKEN',KT) when format is 'FLAT', KT contains a
        ...
        %  the entry.
        %
        %   BioIndexedFile_new(...,'COMMENTPREFIX',CP) specifies a prefix string that
        ...
        %
        %   BioIndexedFile_new(...,'HEADERPREFIX',HP) specifies a prefix string that
        ...
        %  for header lines.
        %
        %   BioIndexedFile_new(...,'TABLEDELIMITER',TD) when format is 'TABLE' or
        ...
        %
        %   BioIndexedFile_new(...,'ENTRYDELIMITER',ED) when format is 'FLAT', ED sets a
        %  string which delimits multiple entries in a source file. ED defaults to
        %  '//'.
        %
        %   BioIndexedFile_new(...,'CONTIGUOUSENTRIES',TF) indicates whether the entries
        ...
        %     read(gene2go,'YAT2')
        %
        %  See also  BioIndexedFile_new,  BioIndexedFile_new.getEntryByIndex,
        %            BioIndexedFile_new.getEntryByKey,  BioIndexedFile_new.getIndexByKey,  
        %            BioIndexedFile_new.getKeys,  BioIndexedFile_new.read            
            ...
            elseif isempty(k)
                error(message('bioinfo: BioIndexedFile_new: BioIndexedFile_new:InvalidFileFormat', fileFormat))
            else
                error(message('bioinfo: BioIndexedFile_new: BioIndexedFile_new:AmbiguousFileFormat', fileFormat))
            end
            ...
            if ~exist(inputFile,'file')
                ...
                error(message('bioinfo: BioIndexedFile_new: BioIndexedFile_new:SourceFileNotFound', [ n, e ], p))
            end
            ...
            switch fileFormat
                ...
                otherwise
                    error(message('bioinfo: BioIndexedFile_new: BioIndexedFile_new:UnsupportedFileFormat'))
            end
            ...
            if ~isdir(p)
                error(message('bioinfo: BioIndexedFile_new: BioIndexedFile_new:InvalidDirectory', p))
            end
            if isempty(n)
            ...
            if ~exist(indexFile,'file')
                ...
                try
                    [nr,nl] =   BioIndexedFile_new.bioindexermex(...
                        indexerType,...
                        inputFile,...
                        indexFile,...
                        indexType,...
                ...
                if nr==0
                    error(message('bioinfo: BioIndexedFile_new: BioIndexedFile_new:indexerError'))
                end
                if verbose
...
%                  
        ...
        function obj = set.Interpreter(obj, h)
            ...
            elseif isempty(h)
                ...
                error(message('bioinfo: BioIndexedFile_new: BioIndexedFile_new:InvalidInterpreter'))
            end
        end
        %==================================================================
        ...
        function obj = set.MemoryMappedIndex(obj, tf)
            try
                ...
                if strcmp(ME.identifier,'bioinfo: BioIndexedFile_new:MemoryMappedIndexOptionNotLogical')
                    error(message('bioinfo: BioIndexedFile_new: BioIndexedFile_new:MemoryMappedIndex'))
                else
                    rethrow(ME)
                end
            end
        ...
        function obj = set.FeatureInMemory(obj, tfs)
            ...
            if ~iscell(tfs) || (~isempty(tfs)&&~isvector(tfs))
                  error(message('bioinfo: BioIndexedFile_new: BioIndexedFile_new:InvalidFeatureInMemory'))
            end
            if numel(obj.FeatureOffset) ~= numel(tfs)
                error(message('bioinfo: BioIndexedFile_new: BioIndexedFile_new:BadSizeFeatureInMemory'))
            end
            try
                for i = 1:numel(tfs)
                   tfs{i} = bioinfoprivate.opttf(tfs{i},'InvalidElementFeatureInMemory',' BioIndexedFile_new: BioIndexedFile_new'); 
                end
            catch ME
                if strcmp(ME.identifier,'bioinfo: BioIndexedFile_new:FeatureInMemoryOptionNotLogical')
                    error(message('bioinfo: BioIndexedFile_new: BioIndexedFile_new:FeatureInMemory'))
        ...
        function str = getEntryByIndex(obj,i)
            ...
            if ~isnumeric(i) || any(i<1) || any(i>obj.NumEntries) || any(rem(i,1))
                error(message('bioinfo: BioIndexedFile_new:getEntryByIndex:nonNumeric'))
            end
        ...
        function keys = getKeys(obj)
            ...
            %
            %  See also  BioIndexedFile_new,  BioIndexedFile_new. BioIndexedFile_new,
            %            BioIndexedFile_new.getEntryByIndex,  BioIndexedFile_new.getEntryByKey,
            %            BioIndexedFile_new.getIndexByKey, BioIndexedFile_new.read
            
            if ~obj.IndexedByKeys
                error(message('bioinfo: BioIndexedFile_new:getIndexByKey:noKeys'))
            end
            keys = (obj.Keys.keys)';
        ...
        function [idx,otoli] = getIndexByKey(obj,key)
            ...
            if ~iscellstr(key)
                error(message('bioinfo: BioIndexedFile_new:getIndexByKey:invalidKey'))
            end
            n = numel(key);
        ...
        function str = getEntryByKey(obj,key)
            ...
            %
            %  See also  BioIndexedFile_new,  BioIndexedFile_new. BioIndexedFile_new,
            %            BioIndexedFile_new.getEntryByIndex,  BioIndexedFile_new.getIndexByKey,
            %            BioIndexedFile_new.getKeys,  BioIndexedFile_new.read
        ...
        function data = read(obj,s)
            ...
            %  See also  BioIndexedFile_new,  BioIndexedFile_new. BioIndexedFile_new,
            %            BioIndexedFile_new.getEntryByIndex,  BioIndexedFile_new.getEntryByKey,
            %            BioIndexedFile_new.getIndexByKey,  BioIndexedFile_new.getKeys
            ...
            elseif iscellstr(s)
                ...
                error(message('bioinfo: BioIndexedFile_new:read:invalidInput'))
            end
        ...
        function obj = getSubset(obj,s)
            ...
            elseif isnumeric(s)&&isvector(s)
                if issorted(s)
                    if any(diff(s)==0)
                        error(message('bioinfo: BioIndexedFile_new:getSubset:invalidNumericInput'))
                    end
            ...
            elseif iscellstr(s)
                if numel(unique(s)) ~= numel(s)
                    error(message('bioinfo: BioIndexedFile_new:getSubset:invalidAlphanumericInput'))
                end
                idx = getIndexByKey(obj,s);
            else
                error(message('bioinfo: BioIndexedFile_new:getSubset:invalidInput'))
            end
            if ~isempty(idx) && any(idx)==0
                    error(message('bioinfo: BioIndexedFile_new:getSubset:logicalIndex'))
            end
            if any(idx>obj.NumEntries)
                    error(message('bioinfo: BioIndexedFile_new:getSubset:outOfBounds'))
            end
            if isempty(obj.SubsetIndex)
                obj.SubsetIndex = idx;
            else
        ...
        function dic = getDictionary(obj)
            ...
            if ~strcmp(obj.FileFormat,'sam')
                error(message('bioinfo: BioIndexedFile_new:getDictionary:invalidFileFormat'))
            end
                ...
                if nargin==2 % All entries indexed by the object are requested:
                    ...
                    elseif isnumeric(s)&&isvector(s)
                        if any(s<1) || any(s>obj.NumEntries) || any(rem(s,1))
                            error(message('bioinfo: BioIndexedFile_new:getFeature:nonNumeric'))
                        else
                            i = s(:);
                        end
                    elseif iscellstr(s)
                        ...
                        error(message('bioinfo: BioIndexedFile_new:getFeature:invalidInput'))
                    end
            ...
            if isscalar(i)
                switch fType
                    ...
                    case 'uint32'
                        fTypeSize = 4;
                    otherwise
                        error(message('bioinfo: BioIndexedFile_new:getFeature:unsupportedFeatureType'))
                end
                fid = fopen(obj.IndexFile,'r');
        ...
        function obj = indexFeature(obj,name)
            ...
            try
                if obj.MemoryMappedIndex || ~isempty(obj.SubsetIndex)
                     BioIndexedFile_new.biofeatureindexermex(...
                        obj.FileFormat,...
                        obj.FeatureName{h},...
                        obj.FeatureType{h},...
                        obj.InputFile,...
            ...
            if (offset == uint64(d.bytes))
                error(message('bioinfo: BioIndexedFile_new:indexFeature:indexerFailure', name, obj.FileFormat))
            else
    ...
    methods (Access = private)
        ...
        function idx = readWholeIndex(obj)
            ...
            fid = fopen(obj.IndexFile,'r');
            if fid==-1
                error(message('bioinfo: BioIndexedFile_new:readWholeIndex:CannotOpenFile', obj.IndexFile))
            end
        ...
        function idx = readIndex(obj,i)
            ...
            if numel(fdu)>100 %if too many file reads, just read once and subindex
                ...
                fid = fopen(obj.IndexFile,'r');
                if fid==-1
                    error(message('bioinfo: BioIndexedFile_new:readIndex:CannotOpenFile', obj.IndexFile))
                end
                if obj.IndexType==1
        ...
        function h = findFeature(obj,name)
            ...
            if isempty(h)
                error(message('bioinfo: BioIndexedFile_new:findFeature:invalidFeature', name, obj.FileFormat))
            end
        end
        %==================================================================
        % readHeaderInfo
        ...
        function obj = readHeaderInfo(obj,wf)
            ...
            fid = fopen(obj.IndexFile,'rb');
            if fid==-1
                error(message('bioinfo: BioIndexedFile_new:readHeaderInfo:CannotOpenFile', obj.IndexFile))
            end
            ...
            if ~strncmp(Magic,sprintf('MBTidx'),6) || (FormatVersion~=1.0 && FormatVersion~=2.0)
                error(message('bioinfo: BioIndexedFile_new:readHeaderInfo:InvalidVersion', obj.IndexFile))
            end
            ...
            if FormatVersion==2.0
                ...
                if wf && obj.IndexedByKeys && (obj.OffsetToKeys==0)
                    warning(message('bioinfo: BioIndexedFile_new: BioIndexedFile_new:noKeysInIndexFile'))
                    obj.IndexedByKeys = false;
                end
                obj.ContiguousEntriesSourceFile = fread(fid,1,'uint8=>double');
                if obj.ContiguousEntriesSourceFile>1
                    error(message('bioinfo: BioIndexedFile_new:readHeaderInfo:InvalidIndexFormat'))
                else
                    obj.ContiguousEntriesSourceFile = ~(obj.ContiguousEntriesSourceFile == 0);
                end
                obj.IndexType = fread(fid,1,'uint8=>uint8');
                if obj.IndexType>1
                    error(message('bioinfo: BioIndexedFile_new:readHeaderInfo:InvalidIndexType'))
                end
            ...
            for i=1:numFeatures
                ...
                if isempty(h)
                    if wf
                        warning(message('bioinfo: BioIndexedFile_new: BioIndexedFile_new:InvalidFeatureName', name, obj.IndexFile, obj.FileFormat))
                    end
                elseif ~strcmp(obj.FeatureType{h},type)
                    if wf
                        warning(message('bioinfo: BioIndexedFile_new: BioIndexedFile_new:InvalidFeatureType', name, obj.IndexFile, type, obj.FeatureType{ h }))
                    end
        ...
        function obj = updateHeaderInfo(obj,name,offset)
            ...
            if fid==-1
                error(message('bioinfo: BioIndexedFile_new:updateHeaderInfo:CannotOpenFile', obj.IndexFile))
            end
            c = onCleanup(@()fclose(fid));
            Magic                 = fread(fid,7,'uint8=>char')';             
            FormatVersion         = sscanf((fread(fid,4,'uint8=>char')'),'%f');
            if ~strncmp(Magic,sprintf('MBTidx'),6) || (FormatVersion~=1.0 && FormatVersion~=2.0)
                error(message('bioinfo: BioIndexedFile_new:updateHeaderInfo:InvalidVersion', obj.IndexFile))
            end
            LockedFile            = ~(fread(fid,1,'uint8=>double') == 0);       %#ok<NASGU>
            obj.NumEntriesSourceFile        = fread(fid,1,'uint32=>double');
        ...
        function range = index2range(obj,i)
            ...
            if obj.ContiguousEntries
                if obj.MemoryMappedIndex
                    ...
                    if ~isempty(obj.SubsetIndex)
                        error(message('bioinfo: BioIndexedFile_new:index2range:ContiguousAndSubset'))
                    end
...
if rem(numel(varargin),2) == 1
    error(message('bioinfo: BioIndexedFile_new: BioIndexedFile_new:IncorrectNumberOfArguments'))
end
    ...
    switch(k)
        ...
        case 3   % interpreter
            ...
            elseif isempty(pval)
                ...
                error(message('bioinfo: BioIndexedFile_new: BioIndexedFile_new:InvalidInterpreterInput'))
            end
        case 11 % verbose
            verbose = bioinfoprivate.opttf(pval,okargs{k},mfilename);
        otherwise
            warning(message('bioinfo: BioIndexedFile_new: BioIndexedFile_new:IgnoredInputs', fileFormat, upper( okargs{ k } )))
    end
end
    ...
    switch(k)
        ...
        case 4   %'keycolumn'
            if isnumeric(pval) && isscalar(pval) && (pval>=0) && (rem(pval,1)==0)
                ...
                error(message('bioinfo: BioIndexedFile_new: BioIndexedFile_new:InvalidColumnWithKeys'))
        ...
        case 5   % keyToken
            if ischar(pval)
                ...
                error(message('bioinfo: BioIndexedFile_new: BioIndexedFile_new:InvalidKeyToken'))
        ...
        case 7   % commentPrefix
            if ischar(pval)
                ...
                error(message('bioinfo: BioIndexedFile_new: BioIndexedFile_new:InvalidCommentPrefix'))
            end
        case 8   % headerPrefix
            if ischar(pval)
                headerPrefix = pval;
            else
                error(message('bioinfo: BioIndexedFile_new: BioIndexedFile_new:InvalidHeaderPrefix'))
            end
        case 9   % tableDelimiter
            if ischar(pval) && (isequal(pval,'\t') || isequal(pval,' ') || isequal(pval,','))
                tableDelimiter = pval;
            else
                error(message('bioinfo: BioIndexedFile_new: BioIndexedFile_new:InvalidTableDelimiter'))
            end
        case 10  % entryDelimiter
            if ischar(pval)
                entryDelimiter = pval;
            else
                error(message('bioinfo: BioIndexedFile_new: BioIndexedFile_new:InvalidEntryDelimiter'))
            end
        case 11 % verbose
            verbose = bioinfoprivate.opttf(pval,okargs{k},mfilename);
    end
